...
- 学習単位: 5漢字=1ステージ、1字につき5回書く（①書き順ガイドなぞり→②数字＋薄グレー→③薄グレーのみ→④⑤自力。2026-08-08のフィードバックで変更。文脈・熟語はテストで出題）。途中でやめても自動で続きから再開
- 文脈問題: 1字につき5パターン（読み/熟語/穴埋め/送り仮名/別読み）。直近出題を記録し連続同一を回避（§11-12）
- ステージテスト5問（出題順・文脈ランダム）+ 期の大型テスト（練習ずみ全漢字・分割なし・途中自動保存・再開可能）（§13, §16）
- テストは正解するまで何度でも書き直せる（×のとき理由表示。認識ミス救済のため。2026-08-08変更）
- 「わからない」ボタン → リスト登録＋答えをなぞってから次へ。リスト解除は**次の正式テストで正解したときだけ**（§14-15）
- 判定のきびしさはプロフィール別に5段階（設定画面。judgeConfig.STRICTNESS_FACTORS）
- 効果音・BGMはWeb Audioで合成（外部音源なし）。各画面右上のスピーカーボタンで一括オンオフ
